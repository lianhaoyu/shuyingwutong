...
function switchTab(tabName) {
    ...
    var allTabs = ['timer', 'records', 'subjects', 'settings'];
...
function setTimerMode(mode) {
    ...
    if (mode === 'tomato') {
        ...
        timerSeconds = studyDuration ? studyDuration.value * 60 : 1500;
    } else if (mode === 'countdown') {
        if (countdownSettings) countdownSettings.style.display = 'block';
        var countdownDuration = document.getElementById('countdown-duration');
        timerSeconds = countdownDuration ? countdownDuration.value * 60 : 10800;
...
function timerComplete() {
    ...
    if (timerMode === 'tomato') {
        if (!isResting) {
            ...
            timerSeconds = restDuration ? restDuration.value * 60 : 300;
            alert('学习结束！开始休息吧！');
            startTimer();
        } else {
            isResting = false;
...
function saveStudyRecord() {
    ...
    if (timerMode === 'countup') {
        ...
        var studyDuration = document.getElementById('study-duration');
        duration = studyDuration ? studyDuration.value * 60 : 1500;
    } else if (timerMode === 'segment') {
        if (segmentTimers.length > 0) {
        ...
        var countdownDuration = document.getElementById('countdown-duration');
        duration = countdownDuration ? countdownDuration.value * 60 : 10800;
    }
...
function updateRecordsList() {
    ...
    var html = '';
    for (var i = 0; i < Math.min(appData.records.length, 20); i++) {
        var record = appData.records[i];
